...
def change():
    ...
    if valid_user(username):
        new_password = input(f'Enter a new password. Note that your password must be at least 5 characters long,'
                             'have an uppercase, and should contain one of these symbols: @, #, $ --->')
        upp = user_register.pass_upper(new_password)
        sym = user_register.pass_symbol(new_password)
        valid = user_register.validate_pass(new_password)

        if valid:
            for key in user_info.keys():
                h_password = hashlib.sha256(new_password.encode()).hexdigest()
                user_info[username] = h_password
                with open('userinfo_files/userinfo.json', 'w') as f:
